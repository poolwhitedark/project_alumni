<template>
	<view class="container999" @touchstart="refreshStart" @touchmove="refreshMove" @touchend="refreshEnd">
		...
		<view class='nav'>
			...
			<view class="typeBox">
				<tj_row flex align="center" >
					<view v-for="(item ,i ) in typeTxt" :key="i" class="tj-col" style="flex: 1;" @click="onActive(i)">
						<view :align="item.align"   :class="['f18',active==i?'mainColor':'']">
							{{item.txt}}
						</view>
		...
		<scroll-view class="bottom" scroll-y="true" @scrolltolower="lower1">
			...
			<view class="noCard" v-if="cardData.length == 0">
				暂无信息
			</view>
			<view class="loadMore" v-if="cardData.length >5">{{noMore == 0 ?(isLoad == 0 ?'加载更多':'加载中'):'暂无更多'}}</view>
		</scroll-view>
	</view>
</template>
...
import tj_row from 'library/row/index.vue';
	export default {
		components: {refresh,tj_card,tj_row},
		data() {
			return {
				list:[], //数据数组
				records:1, //第几页
				...
				typeTxt:[{txt:'推荐',align:'left'},{txt:'热门',align:'center'},{txt:'关注',align:'right'}],
				active:0,
			...
			if(temp == 'refresh') {
			  ...
			  setTimeout(()=>{
				  console.log(`${temp=='refresh'?'刷新请求':'普通请求'}`)
					...
					if(temp == 'refresh'){
						// 刷新结束事件
						setTimeout(()=>{this.$refs.refresh.endAfter();},600)
					}
			  },400)
			},
			onActive(i){
